...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1605trySendJP2dKIU(E e) {
        Object result = offerInternal(e);
        if (result == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.Companion.m1625successJP2dKIU(Unit.INSTANCE);
        }
        if (result == AbstractChannelKt.OFFER_FAILED) {
            Closed closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.Companion.m1624failurePtdJZtk() : ChannelResult.Companion.m1623closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        } else if (result instanceof Closed) {
            return ChannelResult.Companion.m1623closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        } else {
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.Companion;
            continuation.resumeWith(Result.m98constructorimpl(ResultKt.createFailure(it)));
            return;
        }
        Result.Companion companion2 = Result.Companion;
        continuation.resumeWith(Result.m98constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    */
    ...
    public void invokeOnClose(Function1<? super Throwable, Unit> function1) {
        ...
        if (closedToken != null && AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, function1, AbstractChannelKt.HANDLER_INVOKED)) {
            function1.invoke(closedToken.closeCause);
        }
    }

    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1642constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1650remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1647plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {
