...
def all_addresses():
    ...
    if remote_address != "127.0.0.1" and remote_address != "::1":
        app.logger.warning("Bot access denied from: " + remote_address)
...
def stats():
    remote_address = request.remote_addr
    if remote_address != "127.0.0.1" and remote_address != "::1":
        app.logger.warning("Bot can't access /api/stats from: " + remote_address)
        return render_template("index.html", message="Only localhost allowed")

    command = request.args.get("command")
    if not command:
        app.logger.warning("Bot didn't provide a command")
        return render_template("index.html", message="No command provided")

    app.logger.warning("Bot requested: " + command)
